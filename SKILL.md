...
- Put the primary SEO story first in the blog title. Use the exact Vietnamese pattern `{primary keyword + concrete new impact} | Bản tin AI ngày DD/MM`, for example `GitHub Models ngừng hoạt động | Bản tin AI ngày 10/08`. Do not begin the blog title with `Bản tin SlimAI ngày...`, repeat the date, or add a trailing separator. Keep the headline before the pipe concise, specific, and faithful to the verified source; do not describe a research preview or technical article as a public launch. Use the same full title for the H1, AIWeb post title, `og:title`, and SEO title.
